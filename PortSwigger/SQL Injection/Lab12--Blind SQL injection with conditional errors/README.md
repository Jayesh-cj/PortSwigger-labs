...
- Retrieve the ``` administrator ``` Password
    ...
    - Check for the responses with **500 Status Code** ( Internal Server Error ) retrive the characters that replaced in the payload **position 2** in the order of 1 - 20 from the payload position 1, combine all the characters will give the password of ``` administrator ```